...
export class CPU implements CPUTestingComponent {
  ...
  triggerInterrupt(_address: number): void {
  ...
  private executeInstruction(opcode: number): number {
    switch (opcode) {
      ...
      case 0x8e: // ADC A,(HL) - Add memory value at HL to A with carry (Phase 3)
      ...
      case 0x9e: // SBC A,(HL) - Subtract value at HL from A with carry (Phase 4)
  ...
  private executeADDHLBC09(): number {
    ...
    this.setSubtractFlag(false); // N always 0 for ADD
    this.setHalfCarryFlag((hl & 0x0fff) + (bc & 0x0fff) > 0x0fff); // Bit 11->12 carry
  ...
  private executeADDHLDE19(): number {
    ...
    // Calculate and set flags (Z flag preserved for 16-bit ADD)
    this.setSubtractFlag(false); // N always 0 for ADD
    this.setHalfCarryFlag((hl & 0x0fff) + (de & 0x0fff) > 0x0fff); // Bit 11->12 carry
    this.setCarryFlag(result > 0xffff);
  ...
  private executeADDHLHL29(): number {
    ...
    // Calculate and set flags (Z flag preserved for 16-bit ADD)
    this.setSubtractFlag(false); // N always 0 for ADD
    this.setHalfCarryFlag((hl & 0x0fff) + (hl & 0x0fff) > 0x0fff); // Bit 11->12 carry
    this.setCarryFlag(result > 0xffff);
  ...
  private executeADDHLSP39(): number {
    ...
    // Calculate and set flags (Z flag preserved for 16-bit ADD)
    this.setSubtractFlag(false); // N always 0 for ADD
    this.setHalfCarryFlag((hl & 0x0fff) + (sp & 0x0fff) > 0x0fff); // Bit 11->12 carry
    this.setCarryFlag(result > 0xffff);
