...
const spritePool = new ObjectPool(
  ...
  (sprite) => { sprite.x = sprite.y = sprite.tile = sprite.attrs = 0; }
);
```
...
class InstructionSet {
  ...
  private static generateHandlers(): Array<(cpu: CPU) => u32> {
    ...
    handlers[0x00] = (cpu) => { /* NOP */ return 4; };
    handlers[0x01] = (cpu) => {
      // LD BC,nn
...
class PPUMemoryControl {
  ...
  private static readonly OAM_BLOCKED_MODES = (1 << PPUMode.OAM_SEARCH) | (1 << PPUMode.PIXEL_TRANSFER);
...
class MemoryMonitor {
  ...
  private generateRecommendations(): string[] {
    ...
    if (this.heapSize > 50 * 1024 * 1024) { // 50MB threshold
    ...
    if (framebufferAlloc > 5 * 1024 * 1024) { // 5MB threshold
