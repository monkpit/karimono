...
class MockMemoryController implements MemoryController {
  ...
  step(cycles: u32): void { /* mock implementation */ }
  getCurrentCycle(): u32 { return 0; }
  reset(): void { /* mock implementation */ }
}
```
...
describe('CPU Timing Accuracy', () => {
  test('should execute instructions with exact cycle counts', () => {
    const testCases = [
      ...
      { opcode: 0x20, name: 'JR NZ,e', expectedCycles: 12, setup: [0x10], preCondition: () => cpu.setFlag(CPUFlag.ZERO, false) },
      { opcode: 0x20, name: 'JR NZ,e (not taken)', expectedCycles: 8, setup: [0x10], preCondition: () => cpu.setFlag(CPUFlag.ZERO, true) }
    ];
...
describe('PPU Mode Transitions', () => {
  ...
  test('should generate VBlank interrupt at scanline 144', () => {
    ...
    const mockInterruptHandler = {
      requestVBlankInterrupt: () => { vblankInterruptRequested = true; }
    };
...
describe('PPU Rendering', () => {
  test('should render solid color tile correctly', () => {
    ...
    const tileData = new Uint8Array([
      0xFF, 0xFF, // Row 0: 11111111 (all pixels color 3)
      0xFF, 0xFF, // Row 1: 11111111
      0xFF, 0xFF, // Row 2: 11111111
      0xFF, 0xFF, // Row 3: 11111111
      0xFF, 0xFF, // Row 4: 11111111
      0xFF, 0xFF, // Row 5: 11111111
      0xFF, 0xFF, // Row 6: 11111111
      0xFF, 0xFF  // Row 7: 11111111
    ]);
  ...
  test('should handle sprite rendering with priority', () => {
    ...
    const spriteData = new Uint8Array([
      16, 8, 1, 0x00 // Y=0, X=0, Tile=1, Attrs=priority 0
    ]);
...
class MockCartridge implements CartridgeController {
  ...
  // Testing utilities
  enableRAM(enabled: boolean): void { this.ramEnabled = enabled; }
  getCurrentROMBank(): u8 { return this.currentROMBank; }
  setROMBank(bank: u8): void { this.currentROMBank = bank; }
